      ...
      >
        <div class="account-item">
          <div class="account-info">
            ...
            >
              <el-avatar :size="50" :src="user.info?.portraitUrl" />
            </el-badge>
            <div class="info-text">
              <div class="phone">{{ user.phone }}</div>
              <div class="username">{{ user.info?.realName || user.info?.userName || '未知' }}</div>
              <div class="auto-relogin-setting">
                <el-switch
                  v-model="user.auto_relogin"
  ...
  >
    <el-descriptions :column="1" border v-if="currentUser">
      <el-descriptions-item label="用户ID">
        {{ currentUser.info?.id || '未知' }}
      </el-descriptions-item>
      <el-descriptions-item label="手机号">
        {{ currentUser.phone }}
      </el-descriptions-item>
      <el-descriptions-item label="真名">
        {{ currentUser.info?.realName || '未知' }}
      </el-descriptions-item>
      <el-descriptions-item label="用户名">
        {{ currentUser.info?.userName || '未知' }}
      </el-descriptions-item>
      ...
      <el-descriptions-item label="人脸录入">
        <el-tag :type="currentUser.info?.hasFaceKey === 1 ? 'success' : 'warning'">
          {{ currentUser.info?.hasFaceKey === 1 ? '已录入' : '未录入' }}
        </el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="可用社区">
        {{ currentUser.communities?.map(c => `${c.info.communityName} (${c.info.expiryDate})`).join(', ') || '无' }}
      </el-descriptions-item>
    </el-descriptions>
...
import api from '../api'
import type { UserData, LoginForm } from '../types'
...
const addForm = reactive<LoginForm>({
  phone: '',
  code: ''
})
...
const reLoginFormRef = ref<FormInstance | null>(null)
const reLoginForm = reactive<LoginForm>({
  phone: '',
  code: ''
})
...
const reLoginPollingTimer = ref<number | null>(null)
const addPollingTimer = ref<number | null>(null)

const reLoginRules: FormRules = {
  code: [
    { required: true, message: '请输入验证码', trigger: 'blur' },
    { pattern: /^\d{6}$/, message: '请输入6位验证码', trigger: 'blur' }
  ]
...
// 倒计时函数
const startCountdown = (countdownRef: { value: number }): void => {
...
// 发送添加账号验证码
const sendAddCode = async (): Promise<void> => {
  try {
...
// 开始轮询添加账号验证码
const startAddPolling = (): void => {
  stopAddPolling()
  addPollingTimer.value = window.setInterval(async () => {
    try {
      const result = await api.getSmsCode(addForm.phone)
      if (result.code) {
...
const handleAdd = async (): Promise<void> => {
  try {
    ...
    const result = await api.login(addForm.phone, addForm.code)
...
// 显示重新登录对话框
const showReLogin = (user: UserData): void => {
...
// 发送重新登录验证码
const sendReLoginCode = async (): Promise<void> => {
  try {
    sendingReLoginCode.value = true
    
    const data = await api.sendSmsCode(reLoginForm.phone)
...
// 开始轮询重新登录验证码
const startReLoginPolling = (): void => {
  stopReLoginPolling()
  reLoginPollingTimer.value = window.setInterval(async () => {
    try {
      const result = await api.getSmsCode(reLoginForm.phone)
      if (result.code) {
...
const handleReLogin = async (): Promise<void> => {
  try {
    if (!reLoginFormRef.value) return
    await reLoginFormRef.value.validate()
    reLogging.value = true
    
    const result = await api.login(reLoginForm.phone, reLoginForm.code)
...
// 显示详情
const showDetail = (user: UserData): void => {
  currentUser.value = user
  showDetailDialog.value = true
}

// 处理自动重登开关变化
const handleAutoReloginChange = async (user: UserData): Promise<void> => {
  try {
    user.updating = true
    await api.updateAutoRelogin(user.phone, user.auto_relogin || false)
    ElMessage.success(`自动重登已${user.auto_relogin ? '开启' : '关闭'}`)
  } catch (error) {
    console.error('更新自动重登配置失败:', error)
    user.auto_relogin = !user.auto_relogin
